...
final class moojon_adapter extends moojon_base {
	...
	static public function list_tables() {
		...
		foreach (moojon_query_runner::show_tables() as $table) {
			if ($table['Tables_in_'.moojon_config::key('db')] != 'schema_migrations') {
				$tables[] = $table['Tables_in_'.moojon_config::key('db')];
			}
		}
		return $tables;
	}
		...
		foreach (self::list_columns($table) as $column) {
			...
			if ($column['Key'] == 'PRI') {
				...
				switch (strtoupper($type)) {
					...
					case 'SET':
					case 'YEAR':
						throw moojon_exception::create("Data type not mappable ($type)");
						break;
				}
			}
