...
final class moojon_migrator extends moojon_base {
	...
	static public function run() {
		...
		foreach (moojon_files::directory_files(moojon_config::get_migrations_directory()) as $migration_file) {
	...
	static public function roll_back($migration_file, $all = false) {
		self::find_or_create_schema_migrations_table();
		if (schema_migration::read("version = '$migration_file'")->count && $all == false) {
			moojon_base::handle_error("No such migration ($migration_file)");
		}
