<?php
final class moojon_db_driver extends moojon_base {
	protected function __construct() {}
	
	static public function list_tables() {
		$tables = array();
		foreach (moojon_query_runner::show_tables() as $table) {
			if ($table['Tables_in_'.moojon_config::key('db_dbname')] != 'schema_migrations') {
				$tables[] = $table['Tables_in_'.moojon_config::key('db_dbname')];
			}
		}
		return $tables;
	}
